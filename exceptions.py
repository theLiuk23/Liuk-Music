...
class CustomExceptions:
    ...
    @staticmethod
    def return_message(class_name:str):
        return getattr(CustomExceptions, class_name).message
    ...
    class BadArgumentType(commands.CommandError):
        def __init__(self, parameter:str, wrong_type: type, correct_type: type, author:discord.Member=None):
            ...
            self.author = author
            self.message = f"The provided argument '{self.parameter}' is type '{self.wrong_type.__name__}' but it needs to be type '{self.correct_type.__name__}'"
